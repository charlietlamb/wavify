...
export default function SetupUser({user} : UserProps) {
  ...
  async function uploadImage(base64Image: string, contentType: string, user : User): Promise<any> {
  ...
  return (
    <section className="flex flex-col items-center justify-center h-full px-24 min-h-[80vh]">
      <motion.h1 className="account-setup-header font-semibold text-[4rem]"
      initial= {{x: 300, opacity: 0}}
      animate= {{x:0, opacity: 1}}
      transition={{ duration: 1}}>
        Before we begin...
      </motion.h1>
      <div className="user-setup-input flex flex-col w-[60%]">
        <motion.div
        initial={{ x: 300, opacity: 0 }}
        animate={{ x: 0, opacity: 1 }}
        ...
        <motion.div className="flex flex-row items-center justify-between mb-2 input-container gap-x-4"
        initial={{ x: 300, opacity: 0 }}
        animate={{ x: 0, opacity: 1 }}
        transition={{ duration: 1, delay :0.4 }}>
          <Input placeholder="Username" type="text" className="text-center " value={username} onChange={usernameHandler}/>
        ...
        transition={{ duration: 1, delay :0.6 }}>
          <p className="mb-2 text-sm text-muted-foreground">
            A username can only contain lower case characters, numbers, hyphens, underscores and periods
          </p>
